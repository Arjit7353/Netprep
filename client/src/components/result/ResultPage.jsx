// client/src/components/result/ResultPage.jsx
...
import {
  ...
  ChevronLeft, Download, Printer, TrendingUp, TrendingDown,
  Award, Zap, Brain, AlertTriangle, BookOpen, Filter,
  Gauge, Activity, Star, Medal, Hash, Layers, CircleDot,
  FileText, Languages, Minus, ArrowRight
} from 'lucide-react';
import {
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      <svg width={size} height={size} className="transform -rotate-90 drop-shadow-lg">
        ...
        <circle cx={size / 2} cy={size / 2} r={r} stroke="currentColor" className="text-gray-200 dark:text-secondary-700" strokeWidth={strokeWidth} fill="none" />
        <circle cx={size / 2} cy={size / 2} r={r} stroke={`url(#cg-${color?.replace('#', '')})`} strokeWidth={strokeWidth} fill="none" strokeLinecap="round" strokeDasharray={circ} strokeDashoffset={offset} filter="url(#glow)" className="transition-all duration-700 ease-out" />
      </svg>
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-4xl font-black text-gray-900 dark:text-white tracking-tight">{val}<span className="text-2xl">%</span></span>
        {label && <span className="text-sm font-semibold text-gray-600 dark:text-secondary-400 mt-0.5">{label}</span>}
...
const Counter = ({ end, suffix = '', prefix = '' }) => {
  ...
  useEffect(() => {
    let s = 0;
    const inc = Math.max(end / 60, 1);
    const iv = setInterval(() => {
      s += inc;
      if (s >= end) { setC(end); clearInterval(iv); }
...
const GradeBadge = ({ percentage, language }) => {
  const config = percentage >= 90
    ? { grade: 'A+', color: 'from-emerald-500 to-green-600', text: language === 'hi' ? 'उत्कृष्ट!' : 'Outstanding!' }
    : percentage >= 80
      ? { grade: 'A', color: 'from-green-500 to-emerald-600', text: language === 'hi' ? 'बहुत अच्छा!' : 'Excellent!' }
      : percentage >= 70
        ? { grade: 'B+', color: 'from-blue-500 to-cyan-600', text: language === 'hi' ? 'अच्छा प्रदर्शन' : 'Good!' }
        : percentage >= 60
          ? { grade: 'B', color: 'from-cyan-500 to-blue-600', text: language === 'hi' ? 'औसत से ऊपर' : 'Above Average' }
          : percentage >= 50
            ? { grade: 'C', color: 'from-yellow-500 to-amber-600', text: language === 'hi' ? 'औसत' : 'Average' }
            : percentage >= 35
              ? { grade: 'D', color: 'from-orange-500 to-red-500', text: language === 'hi' ? 'सुधार करें' : 'Improve' }
              : { grade: 'F', color: 'from-red-500 to-rose-600', text: language === 'hi' ? 'अभ्यास करें' : 'Practice More' };

  return (
    <div className={`inline-flex items-center gap-3 px-5 py-2.5 rounded-2xl bg-gradient-to-r ${config.color} text-white shadow-lg`}>
...
const StatCard = ({ icon: Icon, label, value, color, gradient }) => (
  <div className="relative overflow-hidden bg-white/80 dark:bg-secondary-800/80 backdrop-blur-sm rounded-2xl border border-gray-100 dark:border-secondary-700 p-4 group hover:shadow-xl transition-all duration-300">
    <div className={`absolute -top-3 -right-3 w-14 h-14 bg-gradient-to-br ${gradient} rounded-full opacity-10 group-hover:opacity-20 transition-opacity`} />
    <div className="flex items-center gap-3">
      <div className={`w-10 h-10 bg-gradient-to-br ${gradient} rounded-xl flex items-center justify-center shadow-sm`}>
        <Icon className="w-5 h-5 text-white" />
      </div>
      <div>
        <p className="text-xs text-gray-500 dark:text-secondary-400 font-medium uppercase tracking-wider">{label}</p>
        <p className={`text-2xl font-black ${color}`}>{typeof value === 'number' ? <Counter end={value} /> : value}</p>
      </div>
    </div>
  </div>
);
...
const LanguageToggle = ({ language, onChange }) => {
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 transition-all text-white text-sm font-medium"
      ...
      <div className="grid grid-cols-10 gap-2">
        {answers.map((a, i) => (
          <button
            ...
            className={`w-full aspect-square rounded-xl border-2 flex items-center justify-center text-xs font-bold cursor-pointer transition-all duration-200 hover:scale-110 hover:shadow-lg ${getStyle(a)} ${a.markedForReview ? 'ring-2 ring-purple-500 ring-offset-1' : ''}`}
            title={`Q.${i + 1}`}
          >
            {i + 1}
          </button>
...
const TimeAnalysis = ({ answers, language }) => {
  const data = useMemo(() => {
    ...
    const fastest = times.filter(t => t.time > 0).sort((a, b) => a.time - b.time)[0];
    const slowest = times.sort((a, b) => b.time - a.time)[0];
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <div key={i} className="bg-white dark:bg-secondary-800 rounded-2xl border border-gray-100 dark:border-secondary-700 p-4 text-center">
            ...
            <p className="text-[11px] text-gray-500 dark:text-secondary-400 mt-0.5">{s.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-secondary-800 rounded-2xl border border-gray-100 dark:border-secondary-700 p-5">
        ...
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={data.times} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
            ...
            <Tooltip contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 8px 30px rgba(0,0,0,0.12)' }} formatter={(v) => [`${v}s`]} />
            <Bar dataKey="time" radius={[6, 6, 0, 0]}>
              {data.times.map((e, i) => <Cell key={i} fill={COLORS[e.status]} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
...
const Insights = ({ attempt, answers, questions, language }) => {
  const insights = useMemo(() => {
    ...
    if (skipped > total * 0.3) list.push({ t: 'warn', i: MinusCircle, m: language === 'hi' ? `${skipped} प्रश्न छोड़ दिए (${Math.round(skipped / total * 100)}%)। सभी attempt करें।` : `${skipped} questions skipped (${Math.round(skipped / total * 100)}%). Try all.` });
    ...
    if (avgT < 30) list.push({ t: 'info', i: Zap, m: language === 'hi' ? `औसत ${Math.round(avgT)}s/प्रश्न। तेज गति! ध्यान से पढ़ें।` : `Avg ${Math.round(avgT)}s/Q. Fast! Read carefully.` });
  ...
  return (
    <div className="space-y-3">
      <h4 className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
        ...
        {language === 'hi' ? 'विश्लेषण और सुझाव' : 'Insights & Tips'}
      </h4>
      {insights.map((ins, i) => (
        <div key={i} className={`flex items-start gap-3 p-4 rounded-2xl border ${styles[ins.t]}`}>
          <ins.i className="w-5 h-5 flex-shrink-0 mt-0.5" />
          <p className="text-sm font-medium">{ins.m}</p>
        </div>
      ))}
    </div>
  );
...
const ResultPage = ({
  ...
  // ═══════════════════════════════════════════════════════════
  //  FULL-SCREEN SOLUTION VIEW — Takes over entire screen
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  //  NORMAL RESULT PAGE (overview, questions, time, topic, compare)
  // ═══════════════════════════════════════════════════════════
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-secondary-900 print:bg-white">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-6 sm:py-8">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
            ...
            <div className="flex-shrink-0 relative">
              ...
              <CircularProgress percentage={scorePct} size={190} strokeWidth={14} color={scoreColor} label={`${score}/${totalMarks}`} sublabel={language === 'hi' ? 'अंक' : 'Marks'} />
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-6 print:hidden">
        <div className="flex overflow-x-auto gap-1 bg-white dark:bg-secondary-800 rounded-2xl p-1.5 border border-gray-100 dark:border-secondary-700 shadow-sm scrollbar-thin">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => {
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-200 ${activeTab === tab.id
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard icon={CheckCircle} label={language === 'hi' ? 'सही उत्तर' : 'Correct'} value={correct} color="text-emerald-600" gradient="from-emerald-500 to-green-600" />
              <StatCard icon={XCircle} label={language === 'hi' ? 'गलत उत्तर' : 'Wrong'} value={wrong} color="text-red-600" gradient="from-red-500 to-rose-600" />
              <StatCard icon={MinusCircle} label={language === 'hi' ? 'छोड़ा गया' : 'Skipped'} value={skipped} color="text-amber-600" gradient="from-amber-500 to-orange-600" />
              <StatCard icon={Target} label={language === 'hi' ? 'सटीकता' : 'Accuracy'} value={`${accuracy}%`} color="text-blue-600" gradient="from-blue-500 to-indigo-600" />
            </div>
              ...
              <div className="bg-white dark:bg-secondary-800 rounded-2xl border border-gray-100 dark:border-secondary-700 p-5 shadow-sm">
                ...
                {(() => {
                  ...
                  const diffData = Object.entries(bd).filter(([, v]) => v.t > 0).map(([k, v]) => ({
                    ...
                    correct: v.c, wrong: v.t - v.c, total: v.t, acc: v.t > 0 ? Math.round(v.c / v.t * 100) : 0,
                    color: k === 'easy' ? '#10B981' : k === 'hard' ? '#EF4444' : '#F59E0B'
                  }));
                  return (
                    <div className="grid grid-cols-3 gap-3 h-[220px] items-end">
                      {diffData.map((d, i) => (
                        <div key={i} className="text-center flex flex-col items-center justify-end h-full">
                          <p className="text-2xl font-black text-gray-900 dark:text-white mb-1">{d.correct}/{d.total}</p>
                          <div className="w-full rounded-t-xl" style={{ height: `${Math.max(d.acc, 10)}%`, backgroundColor: d.color, opacity: 0.2 }}>
                            <div className="w-full rounded-t-xl" style={{ height: `${d.acc}%`, backgroundColor: d.color, minHeight: '4px' }} />
                          </div>
                          <p className="text-sm font-semibold mt-2" style={{ color: d.color }}>{d.name}</p>
                          <p className="text-xs text-gray-500">{d.acc}%</p>
                        </div>
                      ))}
                    </div>
                  );
        ...
        {/* COMPARE */}
        {activeTab === 'compare' && previousAttempts.length > 1 && <ResultComparison attempts={previousAttempts} questions={questions} language={language} />}
      </div>
    </div>
  );
};
