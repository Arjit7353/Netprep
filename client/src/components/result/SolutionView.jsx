...
const TYPE_LABELS = {
  mcq: { en: 'MCQ', hi: 'बहुविकल्पीय' }, assertion_reason: { en: 'Assertion-Reason', hi: 'अभिकथन-कारण' },
  ...
  di_caselet: { en: 'DI-Case', hi: 'DI-केस' }, di_mixed: { en: 'DI-Mixed', hi: 'DI-मिश्र' },
};
const DIFF_LABELS = {
  easy: { en: 'Easy', hi: 'सरल' }, medium: { en: 'Medium', hi: 'मध्यम' }, hard: { en: 'Hard', hi: 'कठिन' },
};
...
const SolutionView = ({
  ...
  useEffect(() => { const t = setTimeout(() => setAnimDir(''), 300); return () => clearTimeout(t); }, [currentIdx]);
  ...
  const filterTabs = [
    ...
    ...(bookmarked.size > 0 ? [{ id: 'bookmarked', label: language === 'hi' ? 'सेव्ड' : 'Saved', count: counts.bookmarked, activeClass: 'bg-amber-600 text-white', icon: Bookmark }] : []),
  ];
    ...
    <div className="fixed inset-0 z-[100] flex flex-col bg-gray-50 dark:bg-secondary-900">
      ...
      <header className="flex-shrink-0 bg-white dark:bg-secondary-800 border-b dark:border-secondary-700 shadow-sm">
        <div className="flex items-center gap-3 px-3 sm:px-5 py-3">
          <button onClick={onClose} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-gray-600 dark:text-secondary-300 hover:bg-gray-100 dark:hover:bg-secondary-700 transition-colors active:scale-95">
          ...
          <div className="hidden md:flex items-center gap-2">
            <span className="flex items-center gap-1 text-emerald-600 font-bold bg-emerald-50 dark:bg-emerald-900/30 px-2.5 py-1 rounded-lg text-sm"><CheckCircle className="w-3.5 h-3.5" /> {counts.correct}</span>
            <span className="flex items-center gap-1 text-red-600 font-bold bg-red-50 dark:bg-red-900/30 px-2.5 py-1 rounded-lg text-sm"><XCircle className="w-3.5 h-3.5" /> {counts.wrong}</span>
            <span className="flex items-center gap-1 text-gray-500 font-bold bg-gray-100 dark:bg-secondary-700 px-2.5 py-1 rounded-lg text-sm"><Circle className="w-3.5 h-3.5" /> {counts.skipped}</span>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden relative" {...swipeHandlers}>
        <main id="sv-scroll" className="flex-1 overflow-y-auto">
          {filteredQ.length === 0 ? (
            ...
            <div className={`max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 pb-8 ${animDir === 'right' ? 'animate-slide-left' : animDir === 'left' ? 'animate-slide-right' : 'animate-fade-in'}`}>
              <QCard question={currentQ} language={language} index={currentIdx} total={filteredQ.length}
                isBookmarked={bookmarked.has(currentQ.questionNumber)}
                onToggleBookmark={() => toggleBookmark(currentQ.questionNumber)}
                showExplanation={showExplanation} onToggleExplanation={() => setShowExplanation(p => !p)} />
        ...
        <aside className="hidden lg:flex flex-col w-72 bg-white dark:bg-secondary-800 border-l dark:border-secondary-700 flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto">
            <PaletteContent allQ={allQ} filteredQ={filteredQ} currentIdx={currentIdx} filter={filter}
              onJump={goToByOriginal} counts={counts} language={language} bookmarked={bookmarked} />
          </div>
        </aside>
      ...
      <footer className="flex-shrink-0 bg-white dark:bg-secondary-800 border-t dark:border-secondary-700 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 max-w-4xl mx-auto">
          <button onClick={goPrev} disabled={currentIdx <= 0}
            ...
            <ChevronLeft className="w-4 h-4" /><span className="hidden sm:inline">{language === 'hi' ? 'पिछला' : 'Previous'}</span>
          </button>
          <div className="flex items-center gap-3">
            <span className="text-sm font-bold text-gray-900 dark:text-white bg-gray-100 dark:bg-secondary-700 px-4 py-2 rounded-xl">
              ...
              <span className="text-gray-400 mx-1">/</span><span>{filteredQ.length}</span>
            </span>
            {currentQ && (
              <button onClick={() => toggleBookmark(currentQ.questionNumber)}
                className={`p-2 rounded-xl transition-all ${bookmarked.has(currentQ.questionNumber)
                  ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600' : 'bg-gray-100 dark:bg-secondary-700 text-gray-400 hover:text-amber-500'}`}>
  ...
  return (
    <div className="p-4 space-y-5">
      <div className="flex flex-wrap gap-3 text-xs">
        ...
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-gray-400" />{language === 'hi' ? 'छोड़ा' : 'Skipped'}</span>
      </div>
        ...
        {allQ.map((q, i) => {
          ...
          return (
            <button key={i} onClick={() => onJump(i)} disabled={disabled}
              className={`relative aspect-square rounded-lg text-xs font-bold flex items-center justify-center transition-all duration-200 ${cc}
...
/* ══════ QUESTION CARD ══════ */
const QCard = ({ question: q, language, index, total, isBookmarked, onToggleBookmark, showExplanation, onToggleExplanation }) => {
  ...
  const rc = q.isCorrect
    ? { icon: CheckCircle, label: language === 'hi' ? 'सही' : 'Correct', cls: 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800', hdr: 'from-emerald-500/5 to-green-500/5' }
    : !isSkipped(q.selectedAnswer)
    ? { icon: XCircle, label: language === 'hi' ? 'गलत' : 'Wrong', cls: 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 border-red-200 dark:border-red-800', hdr: 'from-red-500/5 to-rose-500/5' }
    : { icon: Circle, label: language === 'hi' ? 'छोड़ा' : 'Skipped', cls: 'bg-gray-100 dark:bg-secondary-700 text-gray-600 dark:text-secondary-400 border-gray-200 dark:border-secondary-600', hdr: 'from-gray-500/5 to-slate-500/5' };
  ...
  return (
    <div className="bg-white dark:bg-secondary-800 rounded-2xl shadow-sm border border-gray-200 dark:border-secondary-700 overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`px-2 py-0.5 text-[10px] font-bold rounded uppercase ${
                difficulty === 'easy' ? 'bg-emerald-100 text-emerald-700' : difficulty === 'hard' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
              }`}>{diffLbl}</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {q.timeTaken > 0 && <span className="flex items-center gap-1 text-[11px] font-medium text-gray-500 bg-white dark:bg-secondary-700 px-2.5 py-1 rounded-lg border"><Clock className="w-3 h-3" /> {q.timeTaken}s</span>}
            <button onClick={onToggleBookmark} className={`p-1.5 rounded-lg transition-all ${isBookmarked ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600' : 'bg-gray-100 dark:bg-secondary-700 text-gray-400 hover:text-amber-500'}`}>
      ...
      <div className="p-5 sm:p-6 lg:p-8 space-y-6">
        ...
        <OptsDisplay qData={qData} language={language} selectedAnswer={q.selectedAnswer} correctAnswer={q.correctAnswer} />
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 bg-gray-50 dark:bg-secondary-700/50 rounded-xl border border-gray-100 dark:border-secondary-600">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-sm font-bold px-2.5 py-0.5 rounded-lg ${isSkipped(q.selectedAnswer) ? 'bg-gray-200 text-gray-600' : q.isCorrect ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
              {isSkipped(q.selectedAnswer) ? (language === 'hi' ? 'नहीं दिया' : 'N/A') : `(${optLabel(q.selectedAnswer)})`}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-500">{language === 'hi' ? 'सही:' : 'Correct:'}</span>
            <span className="text-sm font-bold bg-emerald-100 text-emerald-700 px-2.5 py-0.5 rounded-lg">({optLabel(q.correctAnswer)})</span>
          </div>
        </div>
        {qData.explanation && (bText(qData.explanation, language) || bText(qData.explanation, language === 'hi' ? 'en' : 'hi')) && (
          <div>
            <button onClick={onToggleExplanation} className="flex items-center gap-2 text-sm font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-700 mb-3">
              <Lightbulb className="w-4 h-4" />{language === 'hi' ? 'व्याख्या' : 'Explanation'}
            ...
            {showExplanation && (
              <div className="p-5 bg-blue-50/70 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800/30 rounded-xl animate-fade-in">
                <div className="flex items-center gap-2 mb-3">
                  <div className="bg-blue-100 dark:bg-blue-900/30 p-2 rounded-xl"><BookOpen className="w-4 h-4 text-blue-600" /></div>
...
const QContent = ({ qData, language }) => {
  ...
  if (qType === 'passage_based' && qData.passageId) {
    ...
    return (<div className="space-y-4">
      {pc && <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border-l-4 border-amber-400 rounded-r-xl max-h-64 overflow-y-auto"><div className="flex items-center gap-2 mb-2 text-amber-800 font-bold text-xs uppercase"><BookOpen className="w-3.5 h-3.5" />{language === 'hi' ? 'गद्यांश' : 'PASSAGE'}</div><p className="text-sm text-gray-800 dark:text-secondary-200 leading-relaxed whitespace-pre-line">{pc}</p></div>}
      <p className="text-gray-900 dark:text-white font-medium text-lg leading-relaxed">{qt}</p>
    </div>);
  }
  if (qType === 'assertion_reason') {
    const a = bText(qData.assertionReasonData?.assertion, language), r = bText(qData.assertionReasonData?.reason, language);
    return (<div className="space-y-4">
      {qt && <p className="text-gray-700 dark:text-secondary-300 font-medium text-sm">{qt}</p>}
      <div className="flex gap-3 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-800/30"><span className="flex-shrink-0 w-7 h-7 rounded-full bg-blue-600 text-white flex items-center justify-center text-xs font-bold">A</span><div><p className="text-xs font-bold text-blue-700 mb-1 uppercase">{language === 'hi' ? 'अभिकथन' : 'Assertion (A)'}</p><p className="text-sm text-gray-800 dark:text-secondary-200">{a}</p></div></div>
      <div className="flex gap-3 p-4 bg-purple-50 dark:bg-purple-900/10 rounded-xl border border-purple-100 dark:border-purple-800/30"><span className="flex-shrink-0 w-7 h-7 rounded-full bg-purple-600 text-white flex items-center justify-center text-xs font-bold">R</span><div><p className="text-xs font-bold text-purple-700 mb-1 uppercase">{language === 'hi' ? 'कारण' : 'Reason (R)'}</p><p className="text-sm text-gray-800 dark:text-secondary-200">{r}</p></div></div>
    </div>);
  }
  if (qType === 'match_following') {
    const la = bArr(qData.matchData?.listA, language), lb = bArr(qData.matchData?.listB, language);
    return (<div className="space-y-4"><p className="text-gray-900 dark:text-white font-medium text-base">{qt || (language === 'hi' ? 'सुमेलित कीजिए:' : 'Match:')}</p>
      <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-secondary-600"><div className="grid grid-cols-2 bg-gray-100 dark:bg-secondary-700 border-b"><div className="px-4 py-2.5 font-bold text-sm border-r">{language === 'hi' ? 'सूची-I' : 'List-I'}</div><div className="px-4 py-2.5 font-bold text-sm">{language === 'hi' ? 'सूची-II' : 'List-II'}</div></div>
      {la.map((a, i) => (<div key={i} className="grid grid-cols-2 border-b last:border-0"><div className="px-4 py-3 text-sm border-r"><span className="font-bold text-primary-600 mr-1">({optLabel(i)})</span>{a}</div><div className="px-4 py-3 text-sm"><span className="font-bold text-primary-600 mr-1">{roman(i)}</span>{lb[i] || ''}</div></div>))}</div>
      {qData.matchData?.correctMatch && <div className="flex items-center gap-2 p-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 rounded-xl text-sm"><CheckCircle className="w-4 h-4 text-emerald-600" /><span className="font-bold text-emerald-800">{language === 'hi' ? 'सही:' : 'Correct:'}</span><span className="text-emerald-700">{qData.matchData.correctMatch.map((m, i) => `${optLabel(i)}-${roman(m)}`).join(', ')}</span></div>}
    </div>);
  }
  if (qType === 'statement_based') {
    const stmts = bArr(qData.statementData?.statements, language);
    return (<div className="space-y-4"><p className="text-gray-900 dark:text-white font-medium text-base">{qt || (language === 'hi' ? 'कथनों पर विचार:' : 'Consider statements:')}</p>
      <div className="space-y-2">{stmts.map((s, i) => (<div key={i} className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-secondary-700/50 rounded-xl border"><span className="flex-shrink-0 w-6 h-6 rounded-lg bg-white dark:bg-secondary-600 flex items-center justify-center text-xs font-bold">{i+1}</span><span className="text-sm text-gray-800 dark:text-secondary-200 pt-0.5">{s}</span></div>))}</div></div>);
  }
  if (qType === 'sequence_order') {
    const items = bArr(qData.sequenceData?.items, language);
    return (<div className="space-y-4"><p className="text-gray-900 dark:text-white font-medium text-base">{qt || (language === 'hi' ? 'क्रम व्यवस्थित करें:' : 'Arrange:')}</p>
      <div className="space-y-2">{items.map((it, i) => (<div key={i} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-secondary-700/50 rounded-xl border"><span className="flex-shrink-0 w-6 h-6 rounded-lg bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center text-xs font-bold text-primary-700">{optLabel(i)}</span><span className="text-sm text-gray-800 dark:text-secondary-200">{it}</span></div>))}</div></div>);
  }
  if (['di_table','di_bar_chart','di_pie_chart','di_line_graph','di_caselet','di_mixed'].includes(qType) && qData.diDataId) {
    ...
    return (<div className="space-y-4">
      ...
      {bText(di.instruction, language) && <p className="text-sm text-gray-600 italic">{bText(di.instruction, language)}</p>}
      <div className="bg-white dark:bg-secondary-800 border rounded-xl p-4 overflow-x-auto">
        {qType === 'di_table' && <table className="w-full border-collapse text-sm"><thead><tr className="bg-gray-100 dark:bg-secondary-700">{bArr(di.tableData?.headers, language).map((h, i) => <th key={i} className="border px-3 py-2 font-bold text-left">{h}</th>)}</tr></thead><tbody>{(di.tableData?.rows||[]).map((row, ri) => <tr key={ri} className={ri%2?'bg-gray-50 dark:bg-secondary-750':''}>{row.map((c, ci) => <td key={ci} className="border px-3 py-2">{c??'-'}</td>)}</tr>)}</tbody></table>}
        {['di_bar_chart','di_line_graph','di_pie_chart'].includes(qType) && <div className="h-64 w-full min-w-[300px]"><ResponsiveContainer width="100%" height="100%">
          {qType === 'di_bar_chart' ? <BarChart data={bArr(di.chartData?.labels, language).map((l, i) => { const it = {name:l}; (di.chartData?.datasets||[]).forEach((ds, d) => {it[bText(ds.label, language)||`S${d+1}`]=ds.data[i]||0;}); return it; })}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="name" tick={{fontSize:11}}/><YAxis tick={{fontSize:11}}/><Tooltip/><Legend/>{(di.chartData?.datasets||[]).map((ds, i) => <Bar key={i} dataKey={bText(ds.label, language)||`S${i+1}`} fill={ds.color||CHART_COLORS[i%CHART_COLORS.length]} radius={[4,4,0,0]}/>)}</BarChart>
          : qType === 'di_line_graph' ? <LineChart data={bArr(di.chartData?.labels, language).map((l, i) => { const it = {name:l}; (di.chartData?.datasets||[]).forEach((ds, d) => {it[bText(ds.label, language)||`S${d+1}`]=ds.data[i]||0;}); return it; })}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="name" tick={{fontSize:11}}/><YAxis tick={{fontSize:11}}/><Tooltip/><Legend/>{(di.chartData?.datasets||[]).map((ds, i) => <Line key={i} type="monotone" dataKey={bText(ds.label, language)||`S${i+1}`} stroke={ds.color||CHART_COLORS[i%CHART_COLORS.length]} strokeWidth={2}/>)}</LineChart>
          : <PieChart><Pie data={bArr(di.chartData?.labels, language).map((l, i) => ({name:l, value:di.chartData?.datasets?.[0]?.data?.[i]||0}))} cx="50%" cy="50%" outerRadius={80} dataKey="value" label>{bArr(di.chartData?.labels, language).map((_, i) => <Cell key={i} fill={(di.chartData?.datasets?.[0]?.colors||CHART_COLORS)[i%CHART_COLORS.length]}/>)}</Pie><Tooltip/><Legend/></PieChart>}
        </ResponsiveContainer></div>}
        {qType === 'di_caselet' && bText(di.caseletText, language) && <p className="text-sm text-gray-800 leading-relaxed whitespace-pre-line">{bText(di.caseletText, language)}</p>}
      </div>
      <p className="text-gray-900 dark:text-white font-medium text-lg leading-relaxed mt-4">{qt}</p>
    </div>);
  }
  return <p className="text-gray-900 dark:text-white font-medium text-lg leading-relaxed">{qt}</p>;
};
  ...
  return (
    <div className="space-y-3">
      {options.map((opt, i) => {
        ...
        if (isC) { cc = 'border-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 ring-1 ring-emerald-400/50'; ic = <CheckCircle className="w-6 h-6 text-emerald-600" />; tc = 'text-emerald-800 dark:text-emerald-200 font-medium'; }
        else if (isW) { cc = 'border-red-400 bg-red-50 dark:bg-red-900/20 ring-1 ring-red-400/50'; ic = <XCircle className="w-6 h-6 text-red-600" />; tc = 'text-red-800 dark:text-red-200 font-medium'; }
        return (
          <div key={i} className={`flex items-start gap-3 p-4 rounded-xl border-2 transition-all ${cc}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                {isC && <span className="text-[10px] font-bold bg-emerald-600 text-white px-2 py-0.5 rounded-full uppercase">{language === 'hi' ? 'सही' : 'Correct'}</span>}
                {isW && <span className="text-[10px] font-bold bg-red-600 text-white px-2 py-0.5 rounded-full uppercase">{language === 'hi' ? 'आपका' : 'Yours'}</span>}
              </div>
