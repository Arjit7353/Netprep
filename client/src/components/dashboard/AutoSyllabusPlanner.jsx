...
import {
  ...
  X, ExternalLink, Activity, TrendingUp, Star, Filter, Share2, History, AlertTriangle
} from 'lucide-react';
...
const AutoSyllabusPlanner = ({ language = 'en' }) => {
  ...
  // Filters & View State
  const [activeTab, setActiveTab] = useState('schedule'); // 'schedule' or 'backlog'
  const [selectedPaper, setSelectedPaper] = useState('all'); // 'all', 'paper1', 'paper2'
  const [selectedFaculty, setSelectedFaculty] = useState('all'); // 'all', 'Nishant', 'Gulshan', 'Subhangini'
  ...
  // Past Lectures Backlog Calculation
  const backlogLectures = useMemo(() => {
    ...
    MISSION_JRF_SCHEDULE.forEach(item => {
      if (item.date < todayStr) {
        // Paper 1
        if (selectedPaper === 'all' || selectedPaper === 'paper1') {
          (item.paper1 || []).forEach((c, idx) => {
            const key = `off_p1_${item.date}_${idx}`;
            if (!completedMap[key] && (selectedFaculty === 'all' || c.faculty.toLowerCase().includes(selectedFaculty.toLowerCase()))) {
              list.push({
                key,
                paperLabel: 'Paper 1',
                date: item.date,
        ...
        // Paper 2
        if (selectedPaper === 'all' || selectedPaper === 'paper2') {
          (item.paper2 || []).forEach((c, idx) => {
            const key = `off_p2_${item.date}_${idx}`;
            if (!completedMap[key] && (selectedFaculty === 'all' || c.faculty.toLowerCase().includes(selectedFaculty.toLowerCase()))) {
              list.push({
                key,
                paperLabel: 'Paper 2 (History)',
                date: item.date,
                day: item.day,
                unit: c.unit,
                topic: c.topic,
    ...
    return list;
  }, [completedMap, selectedPaper, selectedFaculty]);
  ...
  // Carryover Pending Handler (properly defined at component scope)
  const handleCarryoverPending = useCallback(() => {
  ...
  }, [currentDayData, completedMap, isHi]);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-5 md:p-6 space-y-6">
      ...
      {/* Main Mode Tabs Switcher (Schedule vs Backlog Manager) */}
      <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {isHi ? 'दैनिक कैलेंडर व टू-डू' : 'Schedule & Daily To-Do'}
          </button>
          ...
          >
            ...
            {isHi ? 'पुराने लेक्चर्स बैकलाग (Catch-Up)' : 'Backlog & Past Lectures'}
            {backlogLectures.length > 0 && (
              <span className="px-1.5 py-0.2 text-[10px] font-black bg-red-600 text-white rounded-full ml-1">
                {backlogLectures.length}
              </span>
            )}
          </button>
        </div>
      ...
      {/* VIEW TAB 1: BACKLOG CATCH-UP MANAGER */}
      {activeTab === 'backlog' ? (
        <div className="space-y-4">
          <div className="bg-gradient-to-r from-amber-500 to-red-500 rounded-xl p-4 text-white shadow-md flex items-center justify-between">
            <div className="flex items-center gap-3">
              <History className="w-6 h-6 text-amber-200" />
              <div>
                <h3 className="text-base font-black">
                  {isHi ? 'पुराने लेक्चर्स बैकलाग ट्रैकर (1-Click Catch-Up)' : 'Backlog & Past Lectures Catch-Up Manager'}
                </h3>
                <p className="text-xs text-amber-100">
                  {isHi ? 'छूटे हुए लेक्चर्स को 1-क्लिक में पूरा (Done) मार्क करें और अपनी बैच प्रोग्रेस बढ़ाएं' : 'Mark missed past lectures completed with 1 click to raise your batch progress score'}
                </p>
              </div>
            </div>
            <span className="px-3 py-1 bg-white text-red-600 rounded-lg text-xs font-black">
              {backlogLectures.length} Missed Lectures
            </span>
          </div>

          {backlogLectures.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {backlogLectures.map((item) => (
                <div
                  key={item.key}
                  onClick={() => toggleItemCompleted(item.key)}
                  className="bg-white dark:bg-gray-800 p-3.5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-xs hover:border-amber-400 transition-all cursor-pointer flex items-start gap-3"
                >
                  ...
                  <div className="min-w-0 flex-1 space-y-1">
                    <div className="flex items-center justify-between text-[10px] font-bold text-amber-700 dark:text-amber-300">
                      ...
                      <span className="bg-amber-100 dark:bg-amber-900/40 px-1.5 py-0.2 rounded">{item.paperLabel}</span>
                    </div>
                    <p className="text-xs font-bold text-gray-900 dark:text-white leading-snug">{item.topic}</p>
                    <p className="text-[10px] font-semibold text-gray-500">
                      Unit: {item.unit} • Faculty: {item.faculty} ({item.timing})
                    </p>
                  </div>
                </div>
              ))}
            ...
            <div className="text-center py-12 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800">
              ...
              <h4 className="text-base font-bold text-emerald-800 dark:text-emerald-300">
                {isHi ? 'बधाई हो! आपका कोई भी पुराना लेक्चर छूटा नहीं है!' : 'Zero Backlog! All past lectures are completed!'}
              </h4>
              <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-1">
                {isHi ? 'आप 100% सही समय पर चल रहे हैं।' : 'You are 100% up to date with the Mission JRF batch schedule.'}
              </p>
            </div>
          )}
        </div>
      ) : (
        /* VIEW TAB 2: SCHEDULE & DAILY TO-DO */
        <>
