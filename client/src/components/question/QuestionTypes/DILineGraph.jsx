...
import { TrendingUp, CheckCircle, Circle, AlertCircle } from 'lucide-react';
import {
  getBilingualText,
  getBilingualArray,
  getChartLabels,
...
const DILineGraph = ({
  ...
  return (
    <div className="space-y-4">
      {showDIData && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-3">
            <TrendingUp className="w-5 h-5 text-lime-600 flex-shrink-0" />
                  ...
                  {datasets.map((ds, index) => {
                    ...
                    return (
                      <Line
                        ...
                        type="monotone"
                        dataKey={dsLabel}
                        stroke={ds.color || CHART_COLORS[index % CHART_COLORS.length]}
                        strokeWidth={2.5}
      ...
      {question?.diOrder && (
        <div className="text-xs font-medium text-lime-700 bg-lime-50 px-3 py-1.5 rounded-full inline-block">
          {language === 'hi'
