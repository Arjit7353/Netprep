...
// Find the start of the list variant
const listVariantRegex = /  \/\/ ✨ LIST VARIANT ✨[\s\S]*$/;

const newListVariant = `  // ? LIST VARIANT ? (Advanced UI)
  return (
    <div
      onClick={onCardClick}
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center py-0.5">
          <div className="flex items-center justify-between mb-1.5">
            <div className="flex items-center gap-2 text-xs font-semibold text-gray-500 dark:text-gray-400">
              <span className={\`px-2 py-0.5 rounded bg-gray-100 dark:bg-gray-700 \${t.badgeLight} bg-transparent border \${t.border}\`}>{cfg.title[language]}</span>
              <span>�</span>
          ...
          <h4 className="font-bold text-gray-900 dark:text-white text-base sm:text-lg leading-tight mb-2 pr-4">{test.title}</h4>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-auto">
            ...
            <button onClick={(e) => { e.stopPropagation(); navigate(\`/tests/\${test._id}\`); }}
              className="inline-flex items-center justify-center gap-2 px-5 py-2 rounded-xl text-sm font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 dark:text-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 transition-colors">
              <FileText className="w-4 h-4" /> {language === 'hi' ? '?????' : 'Details'}
            </button>
...
if (listVariantRegex.test(content)) {
  content = content.replace(listVariantRegex, newListVariant + '\n');
  fs.writeFileSync(file, content, 'utf8');
  console.log('Replaced list variant correctly');
} else {
  console.log('Regex did not match list variant');
}
