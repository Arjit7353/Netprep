...
// Get base URL without double /api/
const getBaseUrl = () => {
...
const TestList = ({ language = 'hi' }) => {
  ...
  // Fetch full question objects for all tests
  const fetchAllQuestions = async () => {
    const questionsMap = { ...testQuestions };
    const loadingMap = { ...loadingQuestions };
    const BASE = getBaseUrl();

    const testsToFetch = tests.filter(test => 
      !questionsMap[test._id] || questionsMap[test._id].length === 0
    );
      ...
      testsToFetch.map(async (test) => {
        try {
          ...
          if (test.questions && test.questions.length > 0) {
            ...
            // Case 1: Already full question objects (has question/options fields)
            if (typeof firstQ === 'object' && firstQ !== null && 
                (firstQ.question || firstQ.questionText || firstQ.text || firstQ.options)) {
              fullQuestions = test.questions;
              console.log(`[PDF] "${test.title.substring(0, 35)}..." - Has embedded questions`);
            } 
            // Case 2: Only ObjectIDs - need to fetch full data
            else {
              const questionIds = test.questions.map(q => 
                typeof q === 'string' ? q : (q._id || q.toString())
              );
              
              // Method A: Fetch test with populated questions
              try {
                const response = await fetch(`${BASE}/api/tests/${test._id}`, {
                  headers: {
                ...
                if (response.ok) {
                  ...
                  if (testData.questions && testData.questions.length > 0) {
                    const firstPopulated = testData.questions[0];
                    if (typeof firstPopulated === 'object' && firstPopulated !== null &&
                        (firstPopulated.question || firstPopulated.questionText || firstPopulated.options)) {
                      fullQuestions = testData.questions;
                      console.log(`[PDF] "${test.title.substring(0, 35)}..." - Populated ${fullQuestions.length} Qs`);
                    }
                  }
                }
              } catch (e) {
              ...
              // Method B: Bulk fetch questions
              if (fullQuestions.length === 0 && questionIds.length > 0) {
                try {
                  const response = await fetch(`${BASE}/api/questions/bulk`, {
                    method: 'POST',
                    headers: {
                      'Authorization': `Bearer ${localStorage.getItem('token')}`,
                      'Content-Type': 'application/json'
                  ...
                  if (response.ok) {
                    const data = await response.json();
                    fullQuestions = data.data || data.questions || [];
                    console.log(`[PDF] "${test.title.substring(0, 35)}..." - Bulk fetched ${fullQuestions.length} Qs`);
                  }
                } catch (e) {
                  console.warn('Bulk fetch failed:', e.message);
                }
              }
              
              // Method C: Fetch one by one (fallback, limit to 25)
              if (fullQuestions.length === 0 && questionIds.length > 0 && questionIds.length <= 25) {
                console.log(`[PDF] "${test.title.substring(0, 35)}..." - Fetching individually...`);
                
                const fetched = await Promise.all(
                  questionIds.map(async (qId) => {
                    try {
                      const resp = await fetch(`${BASE}/api/questions/${qId}`, {
                        headers: {
                          'Authorization': `Bearer ${localStorage.getItem('token')}`,
                          'Content-Type': 'application/json'
                        }
                      ...
                      if (resp.ok) {
                        ...
                        return qData.data || qData.question || qData;
                      }
                    } catch {
                      return null;
                    }
                ...
                fullQuestions = fetched.filter(q => q !== null);
                console.log(`[PDF] "${test.title.substring(0, 35)}..." - Got ${fullQuestions.length} Qs`);
              }
            }
          }
          
          questionsMap[test._id] = fullQuestions;
          
          // Debug: show sample question structure
          if (fullQuestions.length > 0) {
            const sample = fullQuestions[0];
            console.log(`[PDF] Sample Q:`, {
              id: sample._id,
              hasQuestion: !!(sample.question || sample.questionText),
              hasOptions: !!sample.options,
              optCount: sample.options?.length || 0,
              type: sample.questionType || 'mcq'
            });
          }
          
        } catch (err) {
          console.error(`[PDF] Failed for "${test.title}":`, err);
  ...
  // Fetch questions for a single test on demand
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (testData.questions && testData.questions.length > 0) {
          ...
          // Check if populated
          if (typeof firstQ === 'object' && (firstQ.question || firstQ.options)) {
            setTestQuestions(prev => ({ ...prev, [testId]: testData.questions }));
            return testData.questions;
          }
          
          // Need to bulk fetch
          const ids = testData.questions.map(q => typeof q === 'string' ? q : q._id);
          const bulkResp = await fetch(`${BASE}/api/questions/bulk`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`,
              'Content-Type': 'application/json'
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-colors"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={loadTests}
              disabled={loading}
              className="p-2.5 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-50"
                ...
                <div key={test._id} className="relative group">
                  ...
                  {loadingQuestions[test._id] && (
                    <div className="absolute top-3 right-12 z-10">
                      <div className="w-8 h-8 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm flex items-center justify-center">
