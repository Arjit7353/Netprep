...
import {
  ...
  BookOpen, Target, Layers, Star,
  CheckCheck, RotateCcw, Sparkles,
  ChevronDown, ChevronUp, SlidersHorizontal,
  Plus, Minus, ChevronFirst, ChevronLast,
  Loader2, ScrollText, BarChart3, PanelLeftClose, PanelLeft,
  Maximize2, Minimize2, Flame, Gauge, Award, Hash, Tag
} from 'lucide-react';
...
const FilterChip = ({ label, onRemove, color = 'gray' }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold border ${colors[color] || colors.gray}`}>
      {label}
      <button type="button" onClick={onRemove} className="p-0.5 hover:bg-black/10 rounded-full"><X className="w-3 h-3" /></button>
    </span>
  );
};
...
// ════════════════════════════════════════
const SimplePreviewModal = ({ question, language, onClose }) => {
  if (!question) return null;
  const t = (h, e) => language === 'hi' ? h : e;
  const text = getQText(question, language);
    ...
    <div className="fixed inset-0 bg-black/60 z-[10000] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-lg">Q{question.questionNumber} — PYQ {question.year}</h3>
          <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-xl"><X className="w-5 h-5" /></button>
        </div>
        <div className="flex flex-wrap gap-1.5 mb-4">
          {question.unit && <span className="text-xs px-2 py-1 bg-blue-100 text-blue-700 rounded-lg">{question.unit}</span>}
          {question.chapter && <span className="text-xs px-2 py-1 bg-green-100 text-green-700 rounded-lg">{question.chapter}</span>}
          {question.topic && <span className="text-xs px-2 py-1 bg-purple-100 text-purple-700 rounded-lg">{question.topic}</span>}
        </div>
        <p className="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap mb-4">{text}</p>
        {question.questionType === 'mcq' && question.options && (
          <div className="space-y-2">
            {(question.options[language] || question.options.hi || question.options.en || []).map((opt, i) => {
              const correct = question.correctAnswer === i;
              return (
                <div key={i} className={`p-3 rounded-xl border-2 text-sm flex items-start gap-2 ${correct ? 'bg-emerald-50 border-emerald-400 text-emerald-800' : 'bg-gray-50 border-gray-200'}`}>
                  <span className={`font-bold w-6 h-6 rounded-full flex items-center justify-center text-xs flex-shrink-0 ${correct ? 'bg-emerald-500 text-white' : 'bg-gray-200'}`}>{String.fromCharCode(65 + i)}</span>
                  <span>{typeof opt === 'string' ? opt : ''}</span>
                </div>
              );
            })}
          </div>
...
const PYQQuestionLibrary = ({
  ...
  const [filters, setFilters] = useState({
    ...
    unitId: '', chapter: '', topic: '',
    type: '', difficulty: '', hasContent: true
  });
  ...
  }, [filters.paper, filters.year, filters.session, filters.unitId, filters.chapter, filters.topic, filters.type, filters.hasContent]);

  const updateFilter = useCallback((key, value) => {
    setFilters(p => {
      const n = { ...p, [key]: value };
      if (key === 'paper') { n.unitId = ''; n.chapter = ''; n.topic = ''; }
      if (key === 'unitId') { n.chapter = ''; n.topic = ''; }
      if (key === 'chapter') { n.topic = ''; }
      return n;
    });
    setPage(1);
  }, []);
  ...
  const clearFilters = useCallback(() => {
    setFilters({ paper: 'paper2', year: '', session: '', unitId: '', chapter: '', topic: '', type: '', difficulty: '', hasContent: true });
    setSearchQuery('');
    setPage(1);
  }, []);
  ...
  const chapterOptions = useMemo(() => {
    if (!pyqFilters?.chapters) return [];
    let chs = pyqFilters.chapters;
    if (filters.unitId) {
      chs = chs.filter(c => c.unitId === filters.unitId);
    }
    return chs;
  }, [pyqFilters, filters.unitId]);

  const topicOptions = useMemo(() => {
    if (!pyqFilters?.topics) return [];
    let tops = pyqFilters.topics;
    if (filters.unitId) {
      tops = tops.filter(t => t.unitId === filters.unitId);
    }
    if (filters.chapter) {
      tops = tops.filter(t => t.chapter === filters.chapter);
    }
    return tops;
  }, [pyqFilters, filters.unitId, filters.chapter]);
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/50 dark:bg-black/70 backdrop-blur-sm z-[9999] flex items-center justify-center p-1.5 sm:p-3">
      <div className={`bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full flex overflow-hidden border border-gray-200/80 dark:border-gray-700 transition-all ${fullscreen ? 'max-w-full h-full rounded-none' : 'max-w-7xl h-[97vh]'}`}>
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {showFilters && (
            <div className="px-4 py-2.5 border-b border-gray-200 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-800/50 flex-shrink-0 space-y-2">
              ...
              {/* Filter dropdowns — Year, Session, Unit, Chapter, Topic, Type */}
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-2">
                ...
                <select value={filters.unitId} onChange={e => updateFilter('unitId', e.target.value)}
                  ...
                  {(pyqFilters?.units || []).map(u => (
                    <option key={u.id} value={u.id}>{u.name} ({u.count})</option>
                  ))}
                </select>

                <select value={filters.chapter} onChange={e => updateFilter('chapter', e.target.value)}
                  disabled={chapterOptions.length === 0}
                  className={`px-3 py-2.5 bg-white dark:bg-gray-800 border-2 rounded-xl text-xs font-medium ${
                    chapterOptions.length === 0
                      ? 'border-gray-100 text-gray-400 cursor-not-allowed'
                      : 'border-gray-200 dark:border-gray-700'
                  }`}>
                  <option value="">{t('सभी अध्याय', 'All Chapters')} {chapterOptions.length > 0 ? `(${chapterOptions.length})` : ''}</option>
                  {chapterOptions.map((c, i) => (
                    <option key={`${c.unitId}_${c.chapter}_${i}`} value={c.chapter}>
                      {c.chapter} ({c.count})
                    </option>
                  ))}
                </select>
                ...
                <select value={filters.topic} onChange={e => updateFilter('topic', e.target.value)}
                  ...
                  <option value="">{t('सभी विषय', 'All Topics')} {topicOptions.length > 0 ? `(${topicOptions.length})` : ''}</option>
                  {topicOptions.map((tp, i) => (
                    <option key={`${tp.topic}_${i}`} value={tp.topic}>
                      {tp.topic} ({tp.count})
                    </option>
                  ))}
                </select>
                ...
                <select value={filters.type} onChange={e => updateFilter('type', e.target.value)}
                  ...
                  {(pyqFilters?.types || []).map(tp => (
                    <option key={tp.type} value={tp.type}>{QUESTION_TYPE_LABELS[tp.type]?.[language] || tp.type} ({tp.count})</option>
                  ))}
                </select>

                {filterCount > 0 && (
                  <button type="button" onClick={clearFilters} className="px-3 py-2.5 text-xs font-bold text-red-600 hover:bg-red-50 rounded-xl flex items-center justify-center gap-1 border-2 border-red-200">
              ...
              {filterCount > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {filters.unitId && <FilterChip label={`Unit: ${(pyqFilters?.units || []).find(u => u.id === filters.unitId)?.name || filters.unitId}`} onRemove={() => updateFilter('unitId', '')} color="green" />}
                  {filters.chapter && <FilterChip label={`Ch: ${filters.chapter}`} onRemove={() => updateFilter('chapter', '')} color="green" />}
                  {filters.topic && <FilterChip label={`Topic: ${filters.topic}`} onRemove={() => updateFilter('topic', '')} color="purple" />}
        ...
        {preview && <SimplePreviewModal question={preview} language={language} onClose={() => setPreview(null)} />}
      </div>
    </div>,
    document.body
  );
