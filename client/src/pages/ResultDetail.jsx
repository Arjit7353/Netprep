...
import {
  ...
  BarChart3, Brain, Timer, Award, Flame, RotateCcw,
  Printer, Sparkles, Zap, AlertTriangle
} from 'lucide-react';
...
const ScoreCircle = ({ percentage, size = 170 }) => {
  ...
  useEffect(() => {
    const t = setTimeout(() => { let c = 0;
      const iv = setInterval(() => { c += 1.2; if (c >= percentage) { setVal(percentage); clearInterval(iv); } else setVal(Math.floor(c)); }, 12);
      return () => clearInterval(iv);
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      <svg width={size} height={size} className="transform -rotate-90 drop-shadow-xl">
        <defs>
          <linearGradient id="scoreG" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor={color}/><stop offset="100%" stopColor={color} stopOpacity="0.4"/></linearGradient>
        ...
        <circle cx={size/2} cy={size/2} r={r} stroke="url(#scoreG)" strokeWidth={12} fill="none" strokeLinecap="round"
          strokeDasharray={circ} strokeDashoffset={offset} filter="url(#scoreGlow)" className="transition-all duration-1000 ease-out"/>
      </svg>
...
const GradeBadge = ({ percentage, language }) => {
  const cfg = percentage >= 90 ? { grade: 'A+', text: language === 'hi' ? 'उत्कृष्ट!' : 'Outstanding!', gradient: 'from-emerald-400 to-green-500', emoji: '🏆' }
    : percentage >= 80 ? { grade: 'A', text: language === 'hi' ? 'बहुत अच्छा!' : 'Excellent!', gradient: 'from-green-400 to-emerald-500', emoji: '🌟' }
    : percentage >= 70 ? { grade: 'B+', text: language === 'hi' ? 'अच्छा' : 'Good', gradient: 'from-blue-400 to-cyan-500', emoji: '👍' }
    : percentage >= 60 ? { grade: 'B', text: language === 'hi' ? 'ठीक है' : 'Above Avg', gradient: 'from-cyan-400 to-blue-500', emoji: '✅' }
    : percentage >= 40 ? { grade: 'C', text: language === 'hi' ? 'सुधार करें' : 'Improve', gradient: 'from-amber-400 to-orange-500', emoji: '📈' }
    : { grade: 'F', text: language === 'hi' ? 'अभ्यास करें' : 'Practice', gradient: 'from-red-400 to-rose-500', emoji: '💪' };
  return (
  ...
  <div className="absolute inset-0 overflow-hidden pointer-events-none">
    {[...Array(8)].map((_, i) => (
      <div key={i} className="absolute w-1 h-1 rounded-full bg-white/20"
        style={{ top: `${10+i*11}%`, left: `${5+i*12}%`, animation: `fp ${3.5+i}s ease-in-out infinite`, animationDelay: `${i*0.5}s` }}/>
    ))}
    <style>{`@keyframes fp{0%,100%{transform:translateY(0) scale(1);opacity:.1}50%{transform:translateY(-25px) scale(1.8);opacity:.35}}`}</style>
  </div>
);
...
const ResultDetail = ({ language = 'en', setLanguage }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setAttempt(aData.data); setReviewData(review.data);
      const tid = review.data?.test?._id;
      if (tid) { try { const p = await attemptService.getAttempts({ testId: tid, limit: 20 }); setPreviousAttempts(p.data || []); } catch { setPreviousAttempts([]); } }
  ...
  // Insights
  const insights = [];
  if (acc >= 80) insights.push({ type: 'success', icon: Award, msg: t(`🏆 Brilliant! ${acc}% accuracy.`, `🏆 शानदार! ${acc}% सटीकता।`) });
  else if (acc >= 50) insights.push({ type: 'info', icon: TrendingUp, msg: t(`👍 Good effort! ${acc}% accuracy.`, `👍 अच्छा! ${acc}% सटीकता।`) });
  else insights.push({ type: 'warn', icon: Brain, msg: t(`📈 ${acc}% accuracy. Focus on fundamentals.`, `📈 ${acc}% सटीकता। मूल अवधारणाओं पर ध्यान दें।`) });
  if (attempt.skippedCount > attempt.correctCount) insights.push({ type: 'warn', icon: MinusCircle, msg: t(`⚠️ ${attempt.skippedCount} skipped. Try all questions.`, `⚠️ ${attempt.skippedCount} छोड़े। सभी attempt करें।`) });
  if (attempt.wrongCount > attempt.correctCount && attempt.wrongCount > 0) insights.push({ type: 'error', icon: XCircle, msg: t(`🚨 Wrong > Correct. Watch negative marking.`, `🚨 गलत > सही। Negative marking का ध्यान रखें।`) });
          ...
          <div className="relative p-6 sm:p-8">
            ...
            <div className="flex items-center justify-between mb-6">
              <button onClick={() => navigate('/results')} className="flex items-center gap-2 text-white/60 hover:text-white transition-colors group">
                <ArrowLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform"/><span className="text-sm font-medium hidden sm:inline">{t('Back', 'वापस')}</span>
              </button>
              <button onClick={() => window.print()} className="p-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-white transition-all backdrop-blur-sm border border-white/10">
            ...
            <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
              ...
              <div className="flex-1 text-center lg:text-left text-white">
                ...
                <div className="flex flex-wrap items-center justify-center lg:justify-start gap-2.5 text-sm text-white/50 mb-5">
                  {[
                    { icon: Clock, text: fmtDate(attempt.completedAt) },
                    { icon: Timer, text: fmtTime(attempt.totalTimeTaken) },
                  ].map((item, i) => (
                    <span key={i} className="flex items-center gap-1.5 bg-white/8 backdrop-blur-sm px-3 py-1.5 rounded-full border border-white/5">
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6">
                  ...
                  {[
                    ...
                    <div key={i}>
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="font-bold text-gray-900 dark:text-white">{bar.val}% {bar.extra && <span className="text-gray-400 text-xs ml-1">({bar.extra})</span>}</span>
                      </div>
                      <div className="h-3 bg-gray-100 dark:bg-secondary-700 rounded-full overflow-hidden">
                        <div className={`h-full bg-gradient-to-r from-${bar.color}-500 to-${bar.color}-600 rounded-full transition-all duration-1000 ease-out shadow-sm shadow-${bar.color}-500/20`}
                          style={{ width: `${bar.val}%` }}/>
                      </div>
                    </div>
                  ))}
                ...
                <Button variant="gradient" fullWidth icon={BookOpen} size="lg" onClick={() => navigate(`/results/${id}/solutions`)} className="mt-4">
                  {t('View Detailed Solutions', 'विस्तृत समाधान देखें')} <ChevronRight className="w-5 h-5"/>
                </Button>
              </div>
            )}
            {activeTab === 'topics' && (
