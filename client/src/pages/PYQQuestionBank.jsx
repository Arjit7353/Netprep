        ...
        {showFilters && (
          <div className="border-t border-gray-100 dark:border-secondary-700 bg-gray-50/60 dark:bg-secondary-900/40 p-4 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div><label className="block text-[10px] font-bold text-gray-500 uppercase mb-1.5 flex items-center gap-1"><ShieldCheck className="w-3 h-3" /> Verification</label><select value={pendingFilters.verificationStatus} onChange={e => handlePendingChange('verificationStatus', e.target.value)} className={selectCls}><option value="">All</option>{Object.entries(VERIFICATION_CONFIG).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select></div>
              <div><label className="block text-[10px] font-bold text-gray-500 uppercase mb-1.5 flex items-center gap-1"><CheckCircle className="w-3 h-3" /> Correctness</label><select value={pendingFilters.correctnessStatus} onChange={e => handlePendingChange('correctnessStatus', e.target.value)} className={selectCls}><option value="">All</option>{Object.entries(CORRECTNESS_CONFIG).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select></div>
            </div>
          ...
          <div className="bg-white dark:bg-secondary-800 rounded-3xl border border-gray-200 dark:border-secondary-700 shadow-2xl w-full max-w-3xl max-h-[92vh] flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              {editTab === 'review' && (<div className="space-y-5">
                ...
                <div><label className="block text-[10px] font-bold text-gray-500 uppercase mb-2 flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5" />Correctness</label>
                ...
                <div><label className="block text-[10px] font-bold text-gray-500 uppercase mb-1.5 flex items-center gap-1"><MessageSquare className="w-3 h-3" />Review Notes</label><textarea value={editForm.reviewNotes||''} onChange={e => handleEditFormChange('reviewNotes', e.target.value)} rows={3} placeholder="Add review notes, corrections needed, etc..." className={textareaCls} /></div>
