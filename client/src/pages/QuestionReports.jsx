...
// FIXED: Dark/Light theme + Language toggle working
...
const normalizePYQQuestion = (question) => {
  ...
  if (!normalized.options || (!normalized.options.hi && !normalized.options.en)) {
    normalized.options = {
  ...
  if (!normalized.assertionReasonData && (question.assertionHi || question.assertion)) {
    normalized.assertionReasonData = {
      assertion: { hi: question.assertionHi || question.assertion || '', en: question.assertionEn || '' },
      reason: { hi: question.reasonHi || question.reason || '', en: question.reasonEn || '' }
    };
  }

  if (!normalized.matchData && (question.listAHi?.length || question.listA?.length)) {
    normalized.matchData = {
      listA: { hi: question.listAHi || question.listA || [], en: question.listAEn || [] },
      listB: { hi: question.listBHi || question.listB || [], en: question.listBEn || [] },
      correctMatch: question.correctMatch || []
    };
  }

  if (!normalized.statementData && (question.statementsHi?.length || question.statements?.length)) {
    normalized.statementData = {
      statements: { hi: question.statementsHi || question.statements || [], en: question.statementsEn || [] },
      correctStatements: question.correctStatements || []
    };
  }

  if (!normalized.sequenceData && (question.itemsHi?.length || question.items?.length)) {
    normalized.sequenceData = {
      items: { hi: question.itemsHi || question.items || [], en: question.itemsEn || [] },
  ...
  return (
    <div className="fixed top-4 right-4 z-[500]" style={{ animation: 'rptSlideIn .3s ease-out' }}>
      <div className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-gradient-to-r ${cfg[type]} text-white shadow-xl text-sm font-medium min-w-[220px]`}>
        ...
        <button onClick={onClose} className="p-0.5 hover:bg-white/20 rounded"><X className="w-3.5 h-3.5" /></button>
      </div>
    </div>
  );
};
...
/* ═══ SKELETON ═══ */
const SkeletonCard = () => (
  <div className="bg-white dark:bg-secondary-800 rounded-xl border border-gray-200 dark:border-secondary-700 p-3.5 space-y-2.5">
    <div className="flex items-center gap-2.5">
      <div className="w-8 h-8 rounded bg-gray-200 dark:bg-secondary-700 rpt-shimmer" />
      <div className="flex-1 space-y-1.5">
        <div className="h-3 rounded bg-gray-200 dark:bg-secondary-700 w-3/4 rpt-shimmer" />
        <div className="h-3 rounded bg-gray-200 dark:bg-secondary-700 w-1/2 rpt-shimmer" />
      </div>
    </div>
  </div>
);
  ...
  <button onClick={onClick}
    className={`relative flex items-center gap-3 p-3 rounded-xl border-2 text-left w-full transition-all duration-200
      ${active
        ...
        : 'bg-white dark:bg-secondary-800 border-gray-200 dark:border-secondary-700 hover:border-gray-300 dark:hover:border-secondary-600 hover:shadow-md'
      }`}>
    <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${active ? 'bg-white/20' : 'bg-gray-100 dark:bg-secondary-700'}`}>
      <IC className={`w-5 h-5 ${active ? 'text-white' : 'text-gray-600 dark:text-secondary-300'}`} />
    </div>
    <div>
      <p className={`text-xl font-black leading-none ${active ? 'text-white' : 'text-gray-900 dark:text-white'}`}>
        <Counter value={value} />
      </p>
      <p className={`text-[10px] font-medium mt-0.5 ${active ? 'text-white/70' : 'text-gray-500 dark:text-secondary-400'}`}>{label}</p>
      {sub && <p className={`text-[9px] ${active ? 'text-white/50' : 'text-gray-400 dark:text-secondary-500'}`}>{sub}</p>}
    </div>
  </button>
);
      ...
      <div className="flex gap-1.5 flex-wrap">
        {items.map((s, i) => (
          <button key={i} onClick={() => setView(s.data)}
            className="w-14 h-14 rounded-lg overflow-hidden border border-gray-200 dark:border-secondary-700 hover:border-blue-400 transition-colors group">
...
const QuestionTypeRenderer = ({ question, language }) => {
  const normalizedQuestion = normalizePYQQuestion(question);
  const qType = (normalizedQuestion?.questionType || 'mcq').toLowerCase().trim();
  ...
  switch (qType) {
    ...
    case 'passage-based':
      return (
        <PassageQuestion
          {...commonProps}
          passage={normalizedQuestion.passageId}
          showPassage={true}
        />
      );
    ...
    case 'di-bar':
    case 'di-bar-chart':
      return <DIBarChart {...commonProps} diData={normalizedQuestion.diDataId} showDIData={true} />;

    case 'di_pie':
    case 'di_pie_chart':
    case 'di-pie':
    case 'di-pie-chart':
      return <DIPieChart {...commonProps} diData={normalizedQuestion.diDataId} showDIData={true} />;

    case 'di_line':
    case 'di_line_graph':
    case 'di-line':
    case 'di-line-graph':
      return <DILineGraph {...commonProps} diData={normalizedQuestion.diDataId} showDIData={true} />;

    case 'di_caselet':
    case 'di-caselet':
    case 'dicaselet':
      return <DICaselet {...commonProps} diData={normalizedQuestion.diDataId} showDIData={true} />;
...
const QuestionCard = ({
  ...
  const startEdit = () => {
    ...
    setEqH(nq.question?.hi || '');
    setEqE(nq.question?.en || '');
    setEoH(
    ...
    setEca(nq.correctAnswer ?? 0);
    setExH(nq.explanation?.hi || '');
    setExE(nq.explanation?.en || '');
  ...
  return (
    <div className={`rounded-xl border transition-all duration-200 ${
      ...
      {/* Header */}
      <div
        className="flex items-center gap-2 px-3 py-2.5 cursor-pointer hover:bg-gray-50 dark:hover:bg-secondary-700/50 transition-colors rounded-xl"
        onClick={() => setOpen(!open)}
      >
        {isReported && <Pin className="w-3 h-3 text-red-500 flex-shrink-0 -rotate-45" />}
        ...
        <p className="flex-1 text-xs text-gray-700 dark:text-secondary-300 truncate min-w-0">
          {qText || '-'}
        </p>
        <div className="flex items-center gap-1 flex-shrink-0">
          <span className="text-[8px] px-1 py-0.5 rounded bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 font-bold uppercase">
            {getTypeLabel(qType)}
          </span>
          <span className="text-[9px] text-gray-400 dark:text-secondary-500 font-mono">
            {nq.correctAnswer >= 0 ? optLabel(nq.correctAnswer) : '-'}
          </span>
          {isReported && (
            <span className="text-[8px] px-1 py-0.5 rounded bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 font-bold">RPT</span>
          )}
          <button
            onClick={(e) => { e.stopPropagation(); startEdit(); }}
            className="p-1 rounded hover:bg-blue-50 dark:hover:bg-blue-900/30 text-gray-400 dark:text-secondary-500 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
          >
            <Edit3 className="w-3 h-3" />
          </button>
          <ChevronDown className={`w-3 h-3 text-gray-400 dark:text-secondary-500 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
        </div>
      </div>

      {/* Expanded */}
      {open && (
        <div className="border-t border-gray-100 dark:border-secondary-700 px-3 py-3 space-y-3 rpt-expand">
          <QuestionTypeRenderer question={nq} language={language} />

          {/* Meta tags */}
          <div className="flex items-center gap-1.5 flex-wrap pt-1 border-t border-gray-100 dark:border-secondary-700">
            {nq.unit && (
              <span className="text-[8px] px-1.5 py-0.5 rounded bg-indigo-50 dark:bg-indigo-900/20 text-indigo-500 dark:text-indigo-400 font-medium">
                {typeof nq.unit === 'object' ? bText(nq.unit, language) : nq.unit}
              </span>
            )}
            {nq.topic && (
              <span className="text-[8px] px-1.5 py-0.5 rounded bg-gray-100 dark:bg-secondary-700 text-gray-500 dark:text-secondary-400 flex items-center gap-0.5">
            ...
            {nq.difficulty && (
              <span className={`text-[8px] px-1.5 py-0.5 rounded font-semibold ${
                nq.difficulty === 'hard' ? 'bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400' :
                nq.difficulty === 'medium' ? 'bg-amber-100 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400' :
                'bg-green-100 dark:bg-green-900/20 text-green-600 dark:text-green-400'
              }`}>{nq.difficulty}</span>
            )}
            {nq.paper && (
              <span className="text-[8px] px-1.5 py-0.5 rounded bg-blue-50 dark:bg-blue-900/20 text-blue-500 dark:text-blue-400 font-medium">
                {nq.paper}
              </span>
            )}
          </div>
          ...
          {editing && (
            <div className="border-t-2 border-blue-200 dark:border-blue-800 pt-3 space-y-2.5">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[10px] font-bold text-gray-700 dark:text-secondary-200 uppercase tracking-wider">
                  {t('संपादक', 'Editor')}
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <div>
                  <label className="text-[9px] font-bold text-gray-400 dark:text-secondary-500 mb-0.5 block uppercase">
                    {t('प्रश्न (हिंदी)', 'Question (HI)')}
                  </label>
                  <textarea value={eqH} onChange={e => setEqH(e.target.value)} rows={2}
                    className="w-full px-2.5 py-1.5 rounded-lg border border-gray-200 dark:border-secondary-600 bg-white dark:bg-secondary-900 text-xs resize-none focus:ring-1 focus:ring-blue-500 outline-none text-gray-800 dark:text-secondary-200" />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-gray-400 dark:text-secondary-500 mb-0.5 block uppercase">
              ...
              <div className="space-y-1.5">
                {['A', 'B', 'C', 'D'].map((lb, i) => (
                  <div key={i} className={`flex items-center gap-1.5 p-1 rounded-lg ${
                    eca === i ? 'bg-emerald-50 dark:bg-emerald-950/20 ring-1 ring-emerald-200 dark:ring-emerald-800' : ''
                  }`}>
                    <button onClick={() => setEca(i)}
                      className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-bold flex-shrink-0 transition-all ${
                        eca === i
                          ...
                          : 'bg-gray-100 dark:bg-secondary-700 text-gray-500 dark:text-secondary-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/20'
                      }`}>
                      {eca === i ? <Check className="w-3 h-3" /> : lb}
                    </button>
                    <input value={eoH[i] || ''} onChange={e => { const n = [...eoH]; n[i] = e.target.value; setEoH(n); }}
                      placeholder={`${lb} (HI)`}
                      className="flex-1 px-2 py-1 rounded border border-gray-200 dark:border-secondary-600 bg-white dark:bg-secondary-900 text-xs focus:ring-1 focus:ring-blue-500 outline-none text-gray-800 dark:text-secondary-200 placeholder:text-gray-400 dark:placeholder:text-secondary-500" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <div>
                  <label className="text-[9px] font-bold text-gray-400 dark:text-secondary-500 mb-0.5 block uppercase">{t('व्याख्या (HI)', 'Expl (HI)')}</label>
                  <textarea value={exH} onChange={e => setExH(e.target.value)} rows={2}
                    className="w-full px-2.5 py-1.5 rounded-lg border border-gray-200 dark:border-secondary-600 bg-white dark:bg-secondary-900 text-xs resize-none focus:ring-1 focus:ring-blue-500 outline-none text-gray-800 dark:text-secondary-200" />
                </div>
                <div>
                  <label className="text-[9px] font-bold text-gray-400 dark:text-secondary-500 mb-0.5 block uppercase">{t('व्याख्या (EN)', 'Expl (EN)')}</label>
              ...
              <div className="flex items-center justify-between pt-2">
                <span className="text-[9px] text-gray-400 dark:text-secondary-500 flex items-center gap-1">
                  <Languages className="w-2.5 h-2.5 text-indigo-500" />
                  {t('ऑटो-ट्रांसलेट + सिंक', 'Auto-translate + Sync')}
                </span>
                <div className="flex gap-1.5">
                  <button onClick={() => setEditing(false)}
                    className="px-2.5 py-1 rounded-lg border border-gray-200 dark:border-secondary-600 text-[10px] font-medium text-gray-500 dark:text-secondary-400 hover:bg-gray-50 dark:hover:bg-secondary-700 transition-colors">
                    {t('रद्द', 'Cancel')}
                  </button>
                  <button onClick={handleSave} disabled={saving}
                    className="px-3 py-1 rounded-lg bg-emerald-500 hover:bg-emerald-600 disabled:bg-gray-300 dark:disabled:bg-secondary-600 text-white text-[10px] font-bold transition-colors flex items-center gap-1">
...
const QuestionReports = ({ language: propLang, setLanguage: propSetLanguage }) => {
  ...
  // ✅ FIX: language को propLang से sync करो — useState नहीं, directly use करो
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-3" style={{ animation: 'rptFadeUp .4s ease-out' }}>
            <div className="flex items-center gap-2.5">
              <button onClick={() => navigate(-1)}
                className="p-2 rounded-xl bg-white dark:bg-secondary-800 border border-gray-200 dark:border-secondary-700 hover:bg-gray-50 dark:hover:bg-secondary-700 transition-colors">
                <ArrowLeft className="w-4 h-4 text-gray-500 dark:text-secondary-400" />
              </button>
            ...
            <button onClick={() => { loadReports(1); loadStats(); }}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-white dark:bg-secondary-800 border border-gray-200 dark:border-secondary-700 text-[10px] font-semibold text-gray-500 dark:text-secondary-400 hover:bg-gray-50 dark:hover:bg-secondary-700 transition-colors">
          ...
          {statsLoading ? (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
              {[1,2,3,4].map(i => <div key={i} className="h-16 rounded-xl bg-gray-200 dark:bg-secondary-700 rpt-shimmer" />)}
            </div>
          ) : stats && (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
              <Stat label={t('लंबित','Pending')} value={stats.pending} icon={Clock}
                grad="bg-gradient-to-br from-amber-500 to-orange-600"
                active={filters.status==='pending'}
                onClick={()=>setFilters(f=>({...f,status:f.status==='pending'?'':'pending'}))} />
              <Stat label={t('समीक्षा','Reviewing')} value={stats.reviewing} icon={Eye}
              ...
              <Stat label={t('ठीक','Fixed')} value={stats.fixed} icon={CheckCircle}
              ...
              <Stat label={t('कुल','Total')} value={stats.total} icon={BarChart3}
                grad="bg-gradient-to-br from-secondary-500 to-secondary-700"
                active={!filters.status}
                sub={`${stats.recentWeek||0} ${t('इस सप्ताह','this week')}`}
          ...
          <div className="bg-white dark:bg-secondary-800 rounded-xl border border-gray-200 dark:border-secondary-700 overflow-hidden">
            <div className="flex items-center gap-2 p-2.5">
              <div className="relative flex-1">
                <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 dark:text-secondary-500" />
                <input value={filters.search} onChange={e => setFilters(f => ({ ...f, search: e.target.value }))}
                  placeholder={t('खोजें...', 'Search...')}
                  className="w-full pl-8 pr-3 py-1.5 rounded-lg border border-gray-200 dark:border-secondary-600 bg-gray-50 dark:bg-secondary-900 text-xs focus:ring-1 focus:ring-blue-500 outline-none text-gray-800 dark:text-secondary-200 placeholder:text-gray-400 dark:placeholder:text-secondary-500" />
              </div>
              <button onClick={() => setShowFilters(!showFilters)}
                className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg border text-[10px] font-semibold transition-all ${
                  showFilters || activeFilters
                    ...
                    : 'border-gray-200 dark:border-secondary-600 text-gray-500 dark:text-secondary-400 hover:bg-gray-50 dark:hover:bg-secondary-700'
                }`}>
                <Filter className="w-3 h-3" />
                {t('फ़िल्टर','Filter')}
                {activeFilters > 0 && (
                  <span className="w-3.5 h-3.5 rounded-full bg-blue-500 text-white text-[8px] font-bold flex items-center justify-center">
                    {activeFilters}
                  </span>
                )}
              </button>
            ...
            {showFilters && (
              <div className="px-2.5 pb-2.5 pt-1 border-t border-gray-100 dark:border-secondary-700 flex flex-wrap gap-2 rpt-expand">
                {[
                  {
                    value: filters.status,
                    onChange: (v) => setFilters(f => ({ ...f, status: v })),
                    options: [
                      { value: '', label: t('सभी स्टेटस','All Status') },
                      { value: 'pending', label: t('लंबित','Pending') },
                      { value: 'reviewing', label: t('समीक्षा','Reviewing') },
                      { value: 'in_progress', label: t('प्रगति में','In Progress') },
                      { value: 'fixed', label: t('ठीक','Fixed') },
                  ...
                  },
                  {
                    ...
                    options: [
                      { value: '', label: t('सभी','All Priority') },
                  ...
                  },
                  {
                    ...
                    options: [
                      ...
                      { value: 'wrong_question', label: t('गलत प्रश्न','Wrong Q') },
                      { value: 'wrong_options', label: t('गलत विकल्प','Wrong Opt') },
                      { value: 'explanation_error', label: t('व्याख्या त्रुटि','Expl Error') },
                  ...
                  <select key={si} value={sel.value} onChange={e => sel.onChange(e.target.value)}
                    className="px-2 py-1 rounded-lg border border-gray-200 dark:border-secondary-600 bg-white dark:bg-secondary-800 text-[10px] outline-none text-gray-700 dark:text-secondary-300 focus:ring-1 focus:ring-blue-500">
                    {sel.options.map(opt => (
          ...
          {selectedReports.size > 0 && (
            <div className="sticky top-0 z-40 bg-white/90 dark:bg-secondary-800/90 backdrop-blur rounded-xl border border-blue-200 dark:border-blue-800 shadow-lg p-2.5 flex items-center gap-2" style={{animation:'rptFadeUp .2s ease-out'}}>
              ...
              <select value={bulkAction} onChange={e => setBulkAction(e.target.value)}
                className="px-2 py-1 rounded border border-gray-200 dark:border-secondary-600 bg-white dark:bg-secondary-800 text-[10px] text-gray-700 dark:text-secondary-300">
              ...
              <button onClick={handleBulk} disabled={!bulkAction}
                className="px-2.5 py-1 rounded bg-blue-500 text-white text-[10px] font-bold disabled:opacity-40 hover:bg-blue-600 transition-colors">
                {t('लागू','Apply')}
              </button>
              <button onClick={() => setSelectedReports(new Set())} className="ml-auto p-1 text-gray-400 dark:text-secondary-500 hover:text-red-500">
                <X className="w-3 h-3" />
              </button>
            </div>
          )}
          ...
          <div className="space-y-2">
            {!loading && reports.length > 0 && (
              <div className="flex items-center justify-between px-1">
                <button onClick={selectAll} className="flex items-center gap-1 text-[10px] text-gray-400 dark:text-secondary-500 hover:text-blue-500 dark:hover:text-blue-400">
                ...
                <span className="text-[10px] text-gray-400 dark:text-secondary-500">
            ...
            {loading ? (
              ...
              <div className="bg-white dark:bg-secondary-800 rounded-xl border border-dashed border-gray-200 dark:border-secondary-700 p-10 text-center">
                <CheckCircle className="w-8 h-8 mx-auto mb-2 text-emerald-400" />
                <p className="text-sm font-bold text-gray-500 dark:text-secondary-400">
                  {t('कोई रिपोर्ट नहीं','No Reports')}
                </p>
              </div>
            ) : reports.map((rpt, idx) => {
              ...
              return (
                <div key={rpt._id}
                  className={`bg-white dark:bg-secondary-800 rounded-xl border-2 overflow-hidden transition-all duration-200
                    ${isExp
                      ? 'border-blue-300 dark:border-blue-700 shadow-lg'
                      : isSel
                        ? 'border-blue-200 dark:border-blue-800'
                        : 'border-gray-200 dark:border-secondary-700 hover:border-gray-300 dark:hover:border-secondary-600 hover:shadow-sm'}
                    ${rpt.priority === 'critical' ? 'ring-1 ring-red-200 dark:ring-red-900/30' : ''}`}
                  style={{ animation: `rptFadeUp .3s ease-out ${idx * 0.03}s both` }}>

                  {/* Row */}
                  <div className="flex items-center gap-2 px-3 py-2.5 cursor-pointer hover:bg-gray-50/50 dark:hover:bg-secondary-700/30 transition-colors"
                    onClick={() => toggleExpand(rpt._id)}>
                    <button onClick={e => { e.stopPropagation(); toggleSelect(rpt._id); }}
                      className={`w-3.5 h-3.5 rounded border flex items-center justify-center flex-shrink-0 ${
                        isSel
                          ? 'bg-blue-500 border-blue-500 text-white'
                          : 'border-gray-300 dark:border-secondary-600 hover:border-blue-400'
                      }`}>
                      {isSel && <Check className="w-2 h-2" />}
                    </button>
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 ${pc.dot}`}
                      style={rpt.priority === 'critical' ? { animation: 'rptDot 2s infinite' } : {}} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1 flex-wrap mb-0.5">
                        ...
                        <span className="text-[9px] text-gray-500 dark:text-secondary-400">
                          {reportService.getReportTypeLabel(rpt.reportType, language)}
                        </span>
                        {rpt.reportCount > 1 && (
                          <span className="px-1 py-0.5 rounded bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-[8px] font-bold">
                            <Users className="w-2 h-2 inline mr-0.5"/>x{rpt.reportCount}
                          </span>
                        )}
                      </div>
                      <p className="text-xs text-gray-700 dark:text-secondary-300 truncate">{rpt.description}</p>
                      <div className="flex items-center gap-2.5 mt-0.5 text-[9px] text-gray-400 dark:text-secondary-500">
                        ...
                        {rpt.affectedTestCount > 0 && (
                          <span className="text-amber-500 font-semibold">
                            <Zap className="w-2 h-2 inline mr-0.5"/>{rpt.affectedTestCount}
                          </span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {rpt.status === 'pending' && (
                        <button onClick={e => { e.stopPropagation(); updateStatus(rpt._id, 'reviewing'); }}
                          className="p-1 rounded bg-blue-50 dark:bg-blue-950/30 text-blue-500 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-950/50 transition-colors">
                          <Eye className="w-3 h-3"/>
                        </button>
                      )}
                      <button onClick={e => { e.stopPropagation(); handleDelete(rpt._id); }}
                        className="p-1 rounded hover:bg-red-50 dark:hover:bg-red-950/20 text-gray-400 dark:text-secondary-500 hover:text-red-500 transition-colors">
                        {deleting === rpt._id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-3 h-3" />}
                      </button>
                      <ChevronDown className={`w-3 h-3 text-gray-400 dark:text-secondary-500 transition-transform duration-200 ${isExp ? 'rotate-180' : ''}`} />
                    </div>
                  </div>

                  {/* Expanded */}
                  {isExp && (
                    <div className="border-t border-gray-100 dark:border-secondary-700 rpt-expand">
                      <div className="px-3 py-3.5 space-y-3.5 bg-gray-50/50 dark:bg-secondary-900/50">
                        {loadingDetail ? (
                          ...
                          <>
                            ...
                            {/* Report Info */}
                            <div className="bg-white dark:bg-secondary-800 rounded-xl border border-gray-200 dark:border-secondary-700 p-3">
                              <h4 className="text-[9px] font-bold text-gray-400 dark:text-secondary-500 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                                <MessageSquare className="w-3 h-3 text-blue-500"/>
                                {t('रिपोर्ट विवरण','Report Details')}
                              </h4>
                              <p className="text-xs text-gray-700 dark:text-secondary-300 leading-relaxed whitespace-pre-wrap">
                                {expandedData.description}
                              </p>
                              {expandedData.suggestedCorrection && (
                                <div className="mt-2 p-2 rounded-lg bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30">
                                  <p className="text-[9px] font-bold text-blue-600 dark:text-blue-400 mb-0.5 flex items-center gap-0.5">
                              ...
                              {expandedData.screenshots?.length > 0 && (
                                <div className="mt-2"><Screenshots items={expandedData.screenshots}/></div>
                              )}
                            </div>
                            ...
                            {(() => {
                              ...
                              if (!hasQ && hasCurrent) {
                                return (
                                  <div className="bg-white dark:bg-secondary-800 rounded-xl border border-gray-200 dark:border-secondary-700 p-3">
                                    <h4 className="text-[9px] font-bold text-gray-400 dark:text-secondary-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                                      <Search className="w-3 h-3 text-blue-500"/>
                                      {t('रिपोर्टेड प्रश्न','Reported Question')}
                                    </h4>
                                    <QuestionCard
                              ...
                              return (
                                <div className="bg-white dark:bg-secondary-800 rounded-xl border border-gray-200 dark:border-secondary-700 overflow-hidden">
                                  <div className="px-3 py-2 bg-gray-50 dark:bg-secondary-700/50 border-b border-gray-100 dark:border-secondary-700">
                                    <h4 className="text-[9px] font-bold text-gray-400 dark:text-secondary-400 uppercase tracking-wider flex items-center gap-1">
                                      <Layers className="w-3 h-3 text-indigo-500"/>
                                      {t('टेस्ट के सभी प्रश्न','All Test Questions')} ({sorted.length})
                                      <span className="text-[8px] font-normal ml-1 text-gray-400 dark:text-secondary-500">
                                        {t('(रिपोर्टेड पहले)','(reported first)')}
                                      </span>
                                    </h4>
                                  </div>
                            ...
                            {expandedData.affectedTests?.length > 0 && (
                              <div className="bg-white dark:bg-secondary-800 rounded-xl border border-gray-200 dark:border-secondary-700 p-3">
                                <h4 className="text-[9px] font-bold text-gray-400 dark:text-secondary-500 uppercase tracking-wider mb-1.5 flex items-center gap-1">
                                  ...
                                  {t('प्रभावित टेस्ट','Affected Tests')} ({expandedData.affectedTests.length})
                                </h4>
                                <div className="space-y-1">
                                  {expandedData.affectedTests.map(ts => (
                                    <div key={ts._id} className="flex items-center justify-between p-1.5 rounded-lg bg-gray-50 dark:bg-secondary-700/50 text-[10px]">
                                      <span className="font-medium text-gray-700 dark:text-secondary-300 truncate">{ts.title}</span>
                                      <span className="px-1 py-0.5 rounded bg-gray-200 dark:bg-secondary-600 text-gray-500 dark:text-secondary-400 text-[8px] font-semibold flex-shrink-0 ml-2">
                                        {ts.testType}
                                      </span>
                                    </div>
                                  ))}
                              ...
                              <div className="flex items-center gap-1.5 flex-wrap">
                                {expandedData.status !== 'reviewing' && (
                                  <button onClick={() => updateStatus(rpt._id, 'reviewing')}
                                    className="px-2.5 py-1 rounded-lg bg-blue-100 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 text-[10px] font-semibold hover:bg-blue-200 dark:hover:bg-blue-950/50 flex items-center gap-0.5 transition-colors">
                                    <Eye className="w-2.5 h-2.5"/>{t('समीक्षा','Review')}
                                  </button>
                                )}
                                <button onClick={() => updateStatus(rpt._id, 'rejected')}
                                  className="px-2.5 py-1 rounded-lg bg-red-100 dark:bg-red-950/30 text-red-700 dark:text-red-400 text-[10px] font-semibold hover:bg-red-200 dark:hover:bg-red-950/50 flex items-center gap-0.5 transition-colors">
                                  <XCircle className="w-2.5 h-2.5"/>{t('अस्वीकृत','Reject')}
                                </button>
                                <button onClick={() => handleDelete(rpt._id)}
                                  className="px-2.5 py-1 rounded-lg border border-gray-200 dark:border-secondary-600 text-gray-500 dark:text-secondary-400 text-[10px] font-semibold hover:bg-red-50 dark:hover:bg-red-950/20 hover:text-red-500 flex items-center gap-0.5 transition-colors">
                              ...
                              <div className="flex items-center gap-2 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800">
                                ...
                                <div>
                                  <p className="text-xs font-bold text-emerald-700 dark:text-emerald-400">
                                    {t('ठीक किया गया','Fixed')}
                                  </p>
                                  {expandedData.fixedAt && (
                                    <p className="text-[9px] text-emerald-600/70 dark:text-emerald-500/70">
                                      {new Date(expandedData.fixedAt).toLocaleString()}
                                    </p>
                                  )}
                                </div>
                              </div>
                          ...
                          <div className="text-center py-4">
                            ...
                            <button onClick={() => toggleExpand(rpt._id)}
                              className="text-[9px] text-blue-500 dark:text-blue-400 mt-1 hover:underline">
                              {t('पुनः प्रयास','Retry')}
                            </button>
                          </div>
                        )}
          ...
          {pagination.pages > 1 && (
            <div className="flex items-center justify-center gap-1.5 pt-2">
              <button disabled={pagination.page <= 1} onClick={() => loadReports(pagination.page - 1)}
                className="p-1.5 rounded-lg border border-gray-200 dark:border-secondary-700 disabled:opacity-30 hover:bg-gray-100 dark:hover:bg-secondary-700 bg-white dark:bg-secondary-800 transition-colors">
                <ChevronLeft className="w-3.5 h-3.5 text-gray-600 dark:text-secondary-400" />
              </button>
                  ...
                  <button key={pg} onClick={() => loadReports(pg)}
                    className={`w-7 h-7 rounded-lg text-[10px] font-bold transition-colors ${
                      pagination.page === pg
                        ...
                        : 'bg-white dark:bg-secondary-800 border border-gray-200 dark:border-secondary-700 text-gray-600 dark:text-secondary-400 hover:bg-gray-50 dark:hover:bg-secondary-700'
                    }`}>
                    {pg}
                  </button>
                );
              ...
              <button disabled={pagination.page >= pagination.pages} onClick={() => loadReports(pagination.page + 1)}
                className="p-1.5 rounded-lg border border-gray-200 dark:border-secondary-700 disabled:opacity-30 hover:bg-gray-100 dark:hover:bg-secondary-700 bg-white dark:bg-secondary-800 transition-colors">
                <ChevronRight className="w-3.5 h-3.5 text-gray-600 dark:text-secondary-400" />
              </button>
            </div>
          )}

          {!loading && pagination.total > 0 && (
            <p className="text-center text-[10px] text-gray-400 dark:text-secondary-500 pb-2">
